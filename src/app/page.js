...
import { Headphones, ShoppingCart, ChevronDownIcon ,ChevronRight,ChevronLeft ,MailIcon, PhoneIcon} from 'lucide-react'
...
function HeroSection() {
  const handleWhatsAppClick = () => {
    const whatsappMessage = encodeURIComponent("היי! הגעתי אליך מהאתר, מה נשמע?");
    const whatsappLink = `https://wa.me/972526383966?text=${whatsappMessage}`;
    window.open(whatsappLink, "_blank");
  };

  return (
    <section className="h-screen flex flex-col justify-center items-center text-center relative overflow-hidden bg-indigo-100">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <Image
          ...
          width={150}
          height={150}
          className="rounded-full mx-auto mb-6 border-4 border-indigo-500 shadow-lg"
        />
        <h1 className="text-4xl font-bold text-indigo-900 mb-4 tracking-wide">
          רועי בנימין גרבר
        </h1>
        <p className="text-lg text-indigo-700 max-w-2xl mx-auto mb-6 leading-relaxed">
          סופר, מתמטיקאי והיסטוריון חובב, בוגר תואר ראשון במדעי המחשב. סגן מפקד ספינה בחיל הים ומפתח תוכנה באמזון.
        </p>
        <Button
          size="lg"
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-full shadow-md transform transition-transform hover:scale-105"
          onClick={() =>
      ...
      <div className="mt-8 flex flex-col items-center space-y-4">
        <p className="text-lg text-gray-600 font-medium">צרו קשר</p>
        <div className="flex space-x-6">
          ...
          <a
            href="mailto:roigerberk@gmail.com"
            className="flex items-center text-gray-700 hover:text-indigo-600 transition"
          >
            <MailIcon className="w-5 h-5 mr-2" />
            roigerberk@gmail.com
          </a>
          <a
            href="https://www.linkedin.com/in/roi-gerber/"
            target="_blank"
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
            >
              <path d="M4.98 3.5C4.98 2.67 5.64 2 6.5 2s1.52.67 1.52 1.5-.68 1.5-1.52 1.5c-.85 0-1.52-.67-1.52-1.5zM5 21h3V8H5v13zm7-6.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5zm-1.5-6h3V8h-3v13h-3V8zm7.5 0h3v1.3c.89-1.2 2.13-1.3 2.13-1.3v3H18v10h-3V10h-3V8h3V6c0-2.33 1.16-4 4-4s4 1.67 4 4v2h-3v2h3v10h-3V8z" />
            </svg>
            LinkedIn
          </a>
        </div>
        <Button
          size="lg"
          className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-full shadow-md transform transition-transform hover:scale-105"
